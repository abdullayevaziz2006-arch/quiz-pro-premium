...
const QuizPage = () => {
  ...
  return (
    <div className="min-h-screen bg-surface py-12 px-6 text-white relative">
      <div className="max-w-5xl mx-auto space-y-10 relative z-10">
        
        <div className="bg-card border border-white/5 p-8 rounded-[40px] flex justify-between items-center gap-10">
          <div className="flex items-center gap-6">
            ...
            <div>
              <p className="text-[10px] uppercase font-bold text-white/40 mb-1">Talaba</p>
          ...
          <div className={`px-8 py-5 rounded-3xl border-2 flex items-center gap-4 transition-all ${timeLeft < 20 ? 'border-primary bg-primary/10 text-primary animate-pulse' : 'border-white/5 bg-white/5'}`}>
            <Timer size={24} />
        ...
        <div className="bg-card border border-white/5 p-12 md:p-20 rounded-[48px] space-y-16 shadow-2xl relative overflow-hidden">
          ...
          <div className="pt-12 border-t border-white/5 flex justify-between items-center relative z-10">
            <button disabled={currentIdx === 0} onClick={() => setCurrentIdx(currentIdx - 1)} className="px-10 py-5 bg-white/5 rounded-2xl text-sm font-bold uppercase tracking-widest disabled:opacity-0 transition-all">Oldingi</button>
            <button onClick={() => currentIdx === questions.length - 1 ? setShowConfirm(true) : (setCurrentIdx(currentIdx + 1), setTimeLeft(settings.timePerQuestion || 120))} className="px-16 py-5 bg-primary rounded-2xl text-sm font-black uppercase tracking-widest shadow-xl shadow-primary/20 transition-all hover:bg-primary/90 flex items-center gap-3">
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6 animate-in fade-in duration-300">
          ...
          <div className="bg-card border border-white/10 p-12 rounded-[48px] max-w-md w-full text-center space-y-8 relative z-10 shadow-2xl">
            ...
            <div className="space-y-2"><h3 className="text-3xl font-black">Yakunlash?</h3><p className="text-white/40">Haqiqatan ham testni yakunlamoqchimisiz?</p></div>
            <div className="flex flex-col gap-3">
              <button onClick={processResult} className="w-full bg-primary py-5 rounded-2xl font-black text-lg shadow-xl shadow-primary/20">HA, YAKUNLAYMAN</button>
              <button onClick={() => setShowConfirm(false)} className="w-full bg-white/5 py-5 rounded-2xl font-bold text-white/40">BEKOR QILISH</button>
            </div>
          </div>
        </div>
      )}
