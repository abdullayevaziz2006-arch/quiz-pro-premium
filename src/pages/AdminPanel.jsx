...
const AdminPanel = () => {
  ...
  if (!isAuthenticated) return (
    <div className="flex items-center justify-center py-20">
      <div className="card-premium max-w-md w-full space-y-6 text-center">
        ...
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); handleLogin(e.target.email.value, e.target.password.value); }}>
          <input className="input-premium" type="email" name="email" placeholder="Email" required />
          <input className="input-premium" type="password" name="password" placeholder="Parol" required />
          <button type="submit" className="btn btn-primary w-full py-4">Kirish</button>
        </form>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-10">
      {toast && (
        <div className={`fixed bottom-10 right-10 z-[2000] px-6 py-3 rounded-xl shadow-2xl flex items-center gap-3 ${
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-4xl font-black">Ranch Admin</h1>
          <p className="text-dim">Boshqaruv markazi</p>
        </div>
      ...
      <main>
        {activeTab === 'questions' && (
          <div className="space-y-8">
            <div className="card-premium flex flex-col md:flex-row justify-between items-center gap-6">
            ...
            <div className="grid gap-4">
              {questions.map((q, idx) => (
                <div key={q.uid} className="card-premium space-y-6">
                  <div className="flex justify-between gap-4">
                    <textarea
                      ...
                      rows={2}
                    />
                    <button onClick={() => setQuestions(questions.filter((_, i) => i !== idx))} className="text-[#FF4D4D] p-2">
                      <Trash2 size={24} />
                    </button>
                  </div>
                  <div className="grid grid-2">
                    {q.options.map((opt, optIdx) => (
                      <div key={optIdx} className={`flex items-center gap-4 p-4 rounded-xl border ${
            ...
            <div className="flex justify-center pt-10">
              <button onClick={() => storage.saveQuestions(adminUid, questions).then(() => showToast("Saqlandi"))} className="btn btn-primary px-16 py-5 text-xl rounded-2xl shadow-2xl">
        ...
        {activeTab === 'sessions' && (
          <div className="space-y-8">
            <div className="card-premium max-w-xl mx-auto space-y-6">
              <h3 className="text-xl font-bold">Test Havolasi Yaratish</h3>
              <input className="input-premium" placeholder="Test nomi..." value={sessionName} onChange={e => setSessionName(e.target.value)} />
              ...
              }} className="btn btn-primary w-full py-4">Yaratish</button>
            </div>

            <div className="grid gap-4">
              {sessions.map(s => (
                <div key={s.id} className="card-premium flex justify-between items-center">
                  <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    }} className="btn btn-secondary px-4">Copy Link</button>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-white/5">
                  <th className="p-6 text-dim text-xs uppercase">Talaba</th>
                  <th className="p-6 text-dim text-xs uppercase">Ball</th>
                  <th className="p-6 text-dim text-xs uppercase">Baho</th>
                </tr>
              </thead>
              <tbody>
                {results.map(r => (
                  <tr key={r.id} className="border-b border-white/5">
                    <td className="p-6 font-bold">{r.student.name} {r.student.surname}</td>
                    <td className="p-6">{r.score}/{r.total}</td>
                    <td className="p-6 font-black text-[#00D68F]">{r.grade}</td>
                  </tr>
                ))}
              </tbody>
            </table>
